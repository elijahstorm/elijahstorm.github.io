 ...
 * https://elijahstorm.github.io/ai/chess/evaluation.js
 */
...
const AI_Actor = new (function () {
  ...
  _codeAiKnobs('easyAi', 2, 4, 0.3)
  _codeAiKnobs('normalAi', 4, 6, 0.2)
  _codeAiKnobs('hardAi', 7, 8, 0.1)
  ...
  function findBestMove(engine, executionStartTime) {
    ...
    if (preferred_lines == null || preferred_lines.length == 0) {
      return null
    }
  ...
  function makeRandomMove(engine) {
    ...
    if (list_of_moves == null || list_of_moves.length == 0) {
      return null
    }
  ...
  function findBookOpening(engine) {
    ...
    let last_move = png_reverse
      .substring(0, png_reverse.indexOf(' '))
      .split('')
      .reverse()
      .join('')
    ...
    let move_in_png_index =
      random_choice.indexOf(last_move) + last_move.length + 1
    ...
    let move = random_choice
      .slice(move_in_png_index, end_move)
      .split('')
      .reverse()
      .join('')
    ...
    if (move.length == 2) {
      ...
      if (move.length == 3 || (move.length == 4 && move[1] == 'x')) {
        ...
        let Knight = [KLU, KRU, KUL, KUR, -KLU, -KRU, -KUL, -KUR]
        ...
        // multiple knights could fit, so the PGN tells us which to move
...
const Engine = new (function () {
  ...
  function commitMoveToEngine(engine, move) {
      ...
      // capturing on en passant
        ...
        if (move.dest == Interface.squareFromXY(file, row)) {
          ...
          for (let i = 0; i < engine.inactive_pieces.length; i++) {
            ...
            if (attack_list.indexOf(enpassant_location) != -1) {
    ...
    for (let i = 0; i < engine.inactive_pieces.length; i++) {
      ...
      if (attack_list.indexOf(engine.status.activeKing) != -1) {
  ...
  function act(move) {
    ...
    if (move.piece == 'N') {
      ...
      for (let i = 0; i < Knight.length; i++) {
        ...
        if (
          Pieces.dereference(current_engine.map[test_dest]) == correct_color
        ) {
    ...
    Canvas.Slide_Piece(current_engine, move, function () {
      ...
      Report.lastMove(
        Style.Beautify_Move(current_engine.status.fullmoves, {
  ...
  function aiResponse(move, callback) {
    if (move == null) {
      // if null, then AI had no legal moves
  ...
  function validateStatusBeforeActing(callback) {
    ...
    let pieces = current_engine.active_pieces.concat(
      current_engine.inactive_pieces
    )
    ...
    if (pieces.length <= 4) {
      ...
      for (let i = 0; i < pieces.length; i++) {
        ...
        if (
          Math.floor(current_engine.map[pieces[i]]) % 16 == 2 ||
      ...
      if (drawn && pieces.length == 4) {
        if (Pieces.dereference(current_engine.map[extraPieces[0]] % 16) == 'B')
          if (
            Pieces.dereference(current_engine.map[extraPieces[1]] % 16) == 'B'
          )
            if (
              Engine.Square_Color(extraPieces[0]) !=
              Engine.Square_Color(extraPieces[1])
            ) {
              // two bishops
              // diff color-squared?
    ...
    for (let i = 0; i < pieces.length; i++) {
      if (
        Pieces.isWhite(current_engine.map[pieces[i]]) !=
        current_engine.status.whiteActive
      )
        continue

      let result = Pieces.All_Possible_Moves(current_engine, pieces[i])
    ...
    if (!at_least_one_possible_move) {
      ...
      if (Engine.In_Check(current_engine)) {
        ...
        Report.status(
          `Checkmate! ${
            current_engine.status.whiteActive ? 'Black' : 'White'
          } wins!`
        )
      } else {
  ...
  function launchAi(callback) {
    ...
    AI_Actor.Solve(function (solution) {
      Interface.Show_AI_Thinking_Icon(false)
      aiResponse(solution, function () {
    ...
    function findNumOfPossibleMoves(engine, depth) {
      ...
      if (depth == 0) {
        for (let p = 0; p < engine.active_pieces.length; p++) {
          ...
          numOfMoves += Pieces.All_Possible_Moves(engine, source).length
        }
      ...
      for (let p = 0; p < engine.active_pieces.length; p++) {
        ...
        let moves = Engine.In_Check(engine)
          ? Pieces.All_Possible_Moves(engine, source)
          : engine.attackers[engine.map[source]]
    ...
    for (let move = 0; move < move_history.length; move++) {
      ...
      if (move_history[move].piece != 'P') {
        ...
        if (move_history[move].captured_piece != null) {
          piece = `${
            'abcdefgh'[Interface.xyFromSquare(move_history[move].source)[0]]
          }`
        }
      }
  ...
  self.In_Check = function (engine) {
    return Engine.Is_Square_Threatened(engine, engine.status.activeKing)
  }
  ...
  self.Engine_From_Fen = function (fen) {
    ...
    engine.status.activeKing = boardStatus.whiteActive
      ? whiteKingLocation
      : blackKingLocation
    engine.status.inactiveKing = boardStatus.whiteActive
      ? blackKingLocation
      : whiteKingLocation
  ...
  self.Generate_Indexable_Data = function (engine) {
    ...
    for (let location = 0; location < engine.map.length; location++) {
      ...
      let isActive =
        Pieces.isWhite(engine.map[location]) == engine.status.whiteActive
      ...
      attackers_and_defenders[engine.map[location]] = move_list.slice()
        ...
        for (let j = 0; j < move_list.length; j++) {
          if (
            ...
            if (
              pin_list.length != 0 &&
              Pieces.dereference(
                engine.map[pin_list[pin_list.length - 1]] % 16
              ) == 'K'
            ) {
  ...
  self.Create_Board = function (fen, no_eval) {
    ...
    if (_runAiTest) runAiAgainstItself()
  ...
  self.create = function (input_engine) {
    ...
    worker.addEventListener('message', function (event) {
      ...
      if (type == 0) {
        ...
        Report.numOfLines(
          `${data.numOfLines} lines evaulated<br />Depth of ${data.depth}`
        )
      ...
      if (Math.abs((square % 8) - file) == 1) {
        if (Math.floor(square / 8) == row - direction * 1) {
          tryToAddToList(
            ...
            list,
            square,
            Interface.squareFromXY(file, 7 - row),
            find_attacks
          )
        }
      }
...
const Style = new (function () {
  ...
  self.Beautify_String = function (move) {
    return `${Pieces.dereference(move.piece)}${String.fromCharCode(
      (move.dest % 8) + 97
    )}${Math.floor(move.dest / 8) + 1}`
  }
  self.Beautify_Move = function (move_num, move, extra_info) {
    ...
    img.src = 'sprites.svg'
    img.style.margin = `${-self.pieceYOffset(move.piece) * scale}px 0 0 ${
      -self.pieceXOffset(move.piece) * scale
    }px`
    ...
    if (move_num != null) move_number.innerHTML = `${move_num}. `

    let move_move = document.createElement('span')
    move_move.innerHTML = `${String.fromCharCode((move.dest % 8) + 97)}${
      Math.floor(move.dest / 8) + 1
    }`
    if (extra_info) {
...
const Sounds = new (function () {
  ...
  self.load = function () {
    ...
    capture = [
      new sound('sfx/capture.mp3'),
      new sound('sfx/capture2.mp3'),
      new sound('sfx/capture3.mp3'),
    ]
    slide = new sound('sfx/slide.mp3')
    place = [
      new sound('sfx/place.mp3'),
      new sound('sfx/place2.mp3'),
      new sound('sfx/place3.mp3'),
      new sound('sfx/place4.mp3'),
    ]
...
const Boards = new (function () {
  ...
  self.middleGameTest =
    '6nr/p2b1kpp/1pq1pp2/n2pP3/P1rP1P2/R1PB3P/4NBP1/Q4RK1 b - - 3 24'
...
const Interface = new (function () {
  ...
  let cur_handler
  let mousedown = false
  let touch_start_loc = new Array(2),
    last_touch_loc = new Array(2)
  ...
  const ___touchstart = function (e) {
    ...
    x = Canvas.inputScale(
      Math.round(e.touches[0].clientX - board.offsetLeft + window.scrollX)
    )
    y = Canvas.inputScale(
      Math.round(e.touches[0].clientY - board.offsetTop + window.scrollY)
    )
  ...
  const ___touchmove = function (e) {
    ...
    if (
      Math.abs(x - touch_start_loc[0]) < 5 &&
  ...
  const ___touchend = function (e) {
    ...
    if (e.timeStamp - __mousedown_time < 170)
      // if released quickly ...
      self.Release(square)
...
const Canvas = new (function () {
  ...
  function init(casing) {
    ...
    document.getElementById('btn-watch-ai').onclick = function () {
      if (Client.Is_Playing()) {
        Client.Stop_Playing()
        document.getElementById('btn-watch-ai').children[0].className =
          'fa fa-gamepad portfolio-ease'
        document.getElementById('btn-watch-ai').children[1].innerHTML = 'Play'
      } else {
        Client.Start_Playing()
        document.getElementById('btn-watch-ai').children[0].className =
          'fa fa-eye portfolio-ease'
        document.getElementById('btn-watch-ai').children[1].innerHTML =
          'Watch AI'
      }
    }
  ...
  function render(engine) {
    ...
    document.getElementById(
      'info-card1'
    ).className = `w3-card w3-round-large w3-padding w3-margin chess-${Style.cardStyle}`
    document.getElementById(
      'info-card2'
    ).className = `w3-card w3-round-large w3-padding w3-margin chess-${Style.cardStyle}`
    ...
    SIZE = Math.min(
      MAXSIZE,
      Math.min(
        Math.floor(window.innerWidth / mobile_flow),
    ...
    if (window.innerWidth < MAXSIZE) {
      ...
      document.getElementById('cards-container').style.width = `${
        window.innerWidth - SIZE * 1.6
      }px`
    ...
    let x = 0,
      ...
      piece,
      tileIndex = 0
    let y_increment = TILESIZE,
      x_increment = TILESIZE
    ...
    for (let i = 0; i < 8; i++) {
      for (let j = 0; j < 8; j++) {
        Context.bg.fillStyle = (i + j) % 2 == 0 ? Style.dark : Style.light
  ...
  function drawHighlight() {
    ...
    if (flip_board) {
      Context.hud.translate(
        Context.hud.canvas.height,
        Context.hud.canvas.height
      )
      Context.hud.scale(-1, -1)
    }

    Context.hud.globalAlpha = 0.6
    ...
    for (let i = 0; i < possible_moves.length; i++) {
      ...
      Context.hud.arc(
        square[0] * TILESIZE + TILESIZE * 0.5,
        square[1] * TILESIZE + TILESIZE * 0.5,
        TILESIZE / 5,
        0,
        2 * Math.PI
      )
  ...
  function recur_slide(
    ...
    setTimeout(function () {
      ...
      recur_slide(piece, location, velocity, interval - 1, TILESIZE, callback)
    }, tpf)
  }
  function recur_fade(piece, x, y, interval, TILESIZE) {
    ...
    setTimeout(function () {
      ...
      recur_fade(piece, x, y, interval - 1, TILESIZE)
    }, tpf)
  }
  ...
  function slide_piece(engine, move, callback) {
    ...
    let speed = [(a * tilesize) / frames, (b * tilesize) / frames]
    ...
    recur_slide(piece, location, velocity, frames, tilesize, function () {
  ...
  self.Flip_Board = function (input) {
    ...
    if (input) {
      document.getElementById('board-eval').className =
        'w3-black w3-round-large chess-eval'
      document.getElementById('eval-black').className = 'w3-white chess-eval'
    } else {
      document.getElementById('board-eval').className =
        'w3-white w3-round-large chess-eval'
