...
export class CinematicBackground {
  constructor(CINEMATIC_ANIMATION = false) {
    const create_path = (sub_path) =>
      `${window.location.origin}/portfolio/display/${sub_path}`
    ...
    function init() {
      ...
      function add_flat_ground() {
        ...
        const mesh = new THREE.Mesh(
          ...
          new THREE.MeshPhongMaterial({ color: 0x009900, depthWrite: false })
        )
        ...
        for (let i = 0; i < 0; i++) {
          loader.load(
            create_path(
              `models/grass/fbx/Grass${Math.floor(Math.random() * 13) + 1}.fbx`
            ),
            function (object) {
              object.position.set(
      ...
      function add_models() {
        const moonTexture = new THREE.TextureLoader().load(
          create_path('images/moon.jpg')
        )
        const normalTexture = new THREE.TextureLoader().load(
          create_path('images/normal.jpg')
        )
    ...
    function start_story() {
      ...
      function moveCamera() {
        ...
        if (CINEMATIC_ANIMATION) {
          ...
          camera.position.z =
            CAMERA_START_POSITION.z + (t * CAMERA_VELOCITY.z) / 10
          camera.position.x =
            CAMERA_START_POSITION.x + (t * CAMERA_VELOCITY.x) / 10
          camera.position.y =
            CAMERA_START_POSITION.y + (t * CAMERA_VELOCITY.y) / 10
        }
      }
      moveCamera()
