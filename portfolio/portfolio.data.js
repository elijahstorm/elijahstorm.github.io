  ...
  show_more = () => {
    ...
    new Promise((resolve, reject) => {
      this.#read_text_file(
        resolve,
        reject,
        `portfolio/port.data.${this.#shown_index}.html`
      )
    }).then(
      (html_content) => {
        ...
        const arrows = document.querySelectorAll(
          '#portfolio-content>div>div.arrow-left'
        )
        arrows.forEach((arrow) => {
          ...
          arrow.onclick = (e) => {
            ...
            for (let index = 0; index < images.length; index++) {
              ...
              const next_index =
                this.#wrap(
                  index + (arrow.classList.contains('flip') ? 1 : -1)
                ) % images.length

              const next = images[next_index]

              const highlights =
                arrow.parentElement.querySelector('.pagination').children
        ...
        const hiddenElements = document.querySelectorAll(
          '#portfolio-content .lazy-slide-in'
        )
